  ...
  return (
    <section>
      ...
      <div id="ads-block" className="card">
        ...
        <p className="muted">PNG/JPG/WEBP/GIF, кликабельная ссылка. Поддерживаются разные пропорции и несколько креативов (ротация на сайте).</p>
        <div className="form-grid two-columns">
          ...
          <label>URL картинок (по одному в строке или через запятую)<textarea rows="3" value={ad.imageUrl || ''} onChange={(e) => setAd({ ...ad, imageUrl: e.target.value })} /></label>
          ...
          <div className="form-action"><button className="btn" onClick={saveAd} disabled={busy}>Сохранить рекламу</button></div>
        </div>
      </div>
      ...
      <div className="card">
        <h3>Загрузки</h3>
        <p className="muted">Всего: {uploadStats.total} · Обработано: {uploadStats.processed} · Ошибки: {uploadStats.failed}</p>
        <div className="table-wrap">
          <table>
            <thead><tr><th>ID</th><th>Файл</th><th>Период</th><th>Статус</th><th>Квитанций</th><th>Неопознано</th><th>Действия</th></tr></thead>
            <tbody>
              {uploads.map((u) => <tr key={u.id}><td>{u.id}</td><td>{u.originalName}</td><td>{u.period}</td><td>{u.status}</td><td>{u.recognizedCount}</td><td>{u.unassignedCount}</td>
