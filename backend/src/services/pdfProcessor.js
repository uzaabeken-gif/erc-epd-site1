...
import { extractAccountFromText, extractAddressFromText, normalizeAccount } from '../utils/validators.js';
...
const getPageSegments = async (buffer) => {
  ...
  for (let i = 1; i <= pdf.numPages; i += 1) {
    ...
    const topAccount = extractAccountFromText(topText, config.accountRegexes);
    const bottomAccount = extractAccountFromText(bottomText, config.accountRegexes);
...
export const processUpload = async (uploadId) => {
  ...
  try {
    ...
    pageSegments.forEach((segment) => {
      const account = extractAccountFromText(segment.text, config.accountRegexes);
      const rawAddress = extractAddressFromText(segment.text, config.addressRegexes);
      if (account) {
